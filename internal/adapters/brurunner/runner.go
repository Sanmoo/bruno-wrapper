...
func (r *runner) Execute(ctx context.Context, req core.RunRequest) (core.Response, error) {
	...
	if readErr != nil {
		if execErr != nil {
			return core.Response{}, fmt.Errorf("bru run failed: %w", execErr)
		}
		return core.Response{}, fmt.Errorf("reading report file: %w", readErr)
	}
	...
	if parseErr != nil {
		...
		return core.Response{}, fmt.Errorf("parsing report: %w", parseErr)
	}

	if len(report.Results) == 0 {
		return core.Response{}, fmt.Errorf("bru report contains no results")
	}
	...
	if result.Status == "error" || result.Status == "skipped" {
		if execErr != nil {
			return core.Response{}, fmt.Errorf("bru run failed (%s): %w", result.Status, execErr)
		}
		return core.Response{}, fmt.Errorf("bru run status: %s", result.Status)
	}

	statusCode, err := result.Response.StatusCode()
	if err != nil {
		return core.Response{}, fmt.Errorf("parsing status code: %w", err)
	}
